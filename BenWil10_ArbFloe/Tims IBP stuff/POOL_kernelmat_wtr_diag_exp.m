function [KMAT,MDIAG]=...
  POOL_kernelmat_wtr_diag_exp(k_wtr,Mmats_u,...
    geom_stuff,LC,ip_stuff,log_stuff,Mmats_v)
...
Nunc=length(hn);
N=length(k_wtr)-1;
Nunc_total=(N+1)*(2*Nunc);
%%
%matrices={R,varTH,TH,R_end};
matrices={R,varTH,TH};
vectors={k_wtr,d2xy_ds2,svec_on_LC};
[U_nonsing,dn0U]=...
...
if ~IF_IRREG
  ...
  KMAT=zeros((N+1)*Nunc,Nunc_total);
  MDIAG=KMAT;
  Mdiag=diag( (.5/LC)./hn );
  %%
  M00left=Mmats_u{1};
  ...
  %%
  for j=0:N
    jvec0m=(1:Nunc)+j*Nunc;%% a^{(j)}_m row
    jvec1m=(1:Nunc)+2*j*Nunc;%% a^{(j)}_n col
    MDIAG(jvec0m,jvec1m)=Mdiag;

    for r=0:N
      jvec0n=(1:Nunc)+r*(2*Nunc);%% a^{(r)}_n column
      jvec1n=jvec0n+Nunc;%% b^{(r)}_n column
