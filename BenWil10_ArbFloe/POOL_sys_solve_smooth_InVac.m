function [Rp,LC]=...
             POOL_sys_solve_smooth_InVac(CMAT,KK,...
               parameters,edge_vars,Nterms,Nmodes_out,Nint,fig,col)
...
% - 23.11.09 - rewritten by LB for in vacuo case - %
...
% v_coeffs=u_coeffs;
w_coeffs=zeros(Nunc,1); %u_coeffs(:,1);
dnw_coeffs=w_coeffs;
xtra_outs=[];
%%
%Nint=300;
...
% - !!!!!!!!!!!!!!!!!!!!!!!!! TEST !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! - %
%ds_dt=inf; d2s_dt2=inf; d2theta_ds2=inf; d2xy_ds2=inf;
% - !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! - %
...
% - FORCING TERMS:
forcing_out=IE_Mat(:,1); %IE_Mat_tilPsi(:,1);
% kap0=k_out(1);
ff=exp(1i*KK*(xvec*cos(theta_inc)+yvec*sin(theta_inc)));
...
for loop_s1=1:Nunc
    ...
    forcing_out(v1) = CMAT*[fm(loop_s1); 0]; %CMAT*[fm(loop_s1); zeros(N+2,1)];

    % v2 = 1:2; 1:N+1; u2 = 1:NG+1; 
    w2 = 1:2; %= N+2:N+3;
...
%     w_inds(loop_s1) = v1(N+2); wn_inds(loop_s1) = v1(N+3);
    ...
    for loop_s2=1:Nunc
        ...
        % - !!!!!!!!!!!!!!!!!!! TEST !!!!!!!!!!!!!!!!!!! - %
%         mat_BS = diag([dnm,dnm]); mat_BS_dn = zeros(2);
        % - !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! - %
...
%         IE_Mat_tilPsi(v1,w2) = IE_Mat_tilPsi(v1,w2) + Mdiag(loop_s1)*tilIw*mat_BS;
        ...
        IE_Mat(v1,w2) = IE_Mat(v1,w2) + Mdiag(loop_s1)*mat_BS; % - From left hand side
        
        w3 = 1:2; %N+2:N+3;

        for loop_s3=1:Nunc % - from integral with Bend and Shear applied
...
%                 M0*tilIw*mat_BS - M1*tilIw*mat_BS_dn;
            ...
            IE_Mat(v1,w3) = IE_Mat(v1,w3) + M0*mat_BS - M1*mat_BS_dn;
            
            w3=w3+2; %N+3;

        end

        %v2=v2+N+3; u2=u2+NG+1; 
        w2=w2+2; %N+3;
    end
...
% % - size 1 x Nunc*(N+3)
w_inds=1:2:2*Nunc; wn_inds=2:2:2*Nunc; 
% w_I = forcing_out(w_inds); %tilPsi_I(w_inds,:);
% wn_I = forcing_out(wn_inds); %tilPsi_I(wn_inds,:);
...
coeffs = IE_Mat\forcing_out;

w_coeffs = coeffs(w_inds); %w_I + w_u*u_vec;
w_n_coeffs = coeffs(wn_inds); %wn_I + + wn_u*u_vec;

%% get the coefficients of del2w & del2w_n:
what_coeffs=( MB1*w_coeffs+MB2*w_n_coeffs );
what_n_coeffs=( MS1*w_coeffs+MS2*w_n_coeffs );
% - TEST - %
% what_coeffs=w_n_coeffs;
% w_n_coeffs=0*w_n_coeffs; what_n_coeffs = w_n_coeffs;
...
for loop_s=-Nterms:Nterms
    chi_mat(Nterms+1+loop_s,:) = exp(1i*loop_s*pi*svec/LC);
end
...
title('w_s')
plot(svec/LC, abs((1i*pi/LC)*w_coeffs.'*diag([-Nterms:Nterms])*chi_mat),col);
clear chi_mat col
elseif 0
figure(fig); hold on
for loop_s=-Nterms:Nterms
...
Rp=zeros(2,Nunc); %zeros(N+3,Nunc);
Sp=Rp;
ddt=zeros(2); %zeros(N+3,N+3);
ddt(1)=1;
%%
% a_hat=-invCMAT*[Psi;w_coeffs.'; what_coeffs.'];
% b_hat=invCMAT*[Psi_n;w_n_coeffs.'; what_n_coeffs.'];
a_hat=-invCMAT*[w_coeffs.'; what_coeffs.'];
b_hat=invCMAT*[w_n_coeffs.'; what_n_coeffs.'];
...
besj=besselj(jj,KK*rr_pol); %besselj(jj,kap0*rr_pol);
dr_besj=KK/2*( besj(:,jj0)-besj(:,jj0+2) ); %kap0/2*( besj(:,jj0)-besj(:,jj0+2) );
besj=besj(:,jj0+1);
%%
pp=jj(jj0+1);
Exp=exp( -1i*th_pol*pp );
dExp=exp( -1i*th_pol*pp )*diag(-1i*pp );
%%
Jp=besj.*Exp;
