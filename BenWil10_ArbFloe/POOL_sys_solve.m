function [a_tilde,b_tilde,Rp,LC,svec,dtheta_ds]=...
             POOL_sys_solve(matrices,vectors,...
               parameters,edge_vars,N,Nterms,Nmodes_out, Nint)
           
% - 02.06.09 - modified by LB for corners - %
...
if USE_EXP%% exponentials:
  ...
  hn=hn*LC;
  %% - 03.06.09 - LB new weighting
%   rad = edge_vars{3}{1}(1);
%   wt =  sqrt((1/4i)*besselj(-Nterms:Nterms, vectors{1}*rad).*besselh(-Nterms:Nterms, vectors{1}*rad)); clear rad % ones(1,2*Nterms+1); %
%   wt = abs(wt);
%   hn = hn.*(wt.');
%   IP = diag(1./wt)*IP; %clear wt
  %%
...
%log_corr_mn=diag(ln_coeff./(wt.^2).')+IP*log_corr*IP';
...
disp(['condition number=',num2str(abs(rcond(KMAT)))]);
